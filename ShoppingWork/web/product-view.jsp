...
<div id="header" class="wrap">
	...
	<div class="help">
		<c:if test="${
		name!=null}"><a href="selectdd?dd=${name.userName }">个人订单</a></c:if>
		<c:if test="${name!=null}">当前用户${name.userId }</c:if><a href="ShopSelect" class="shopping">购物车</a>
		<c:if test="${name==null}"><a href="login.jsp">登录</a>|<a href="register.jsp">注册</a></c:if>
		<c:if test="${name!=null}"><a href="zx">退出</a></c:if><a href="SelallServlet">留言</a>
		<c:if test="${name.status==2}"><a href="manage/index.jsp" >去后台</a></c:if>
	</div>
	<div class="navbar">
		<ul class="clearfix">
			<c:choose>
			...
			<c:choose>
			<c:when test="${selected_fid == f.pcId}">
			<li class="current" ><a href="selectProductList?fid=${f.pcId }">${f.pcName }</a></li>
			</c:when>
			<c:otherwise>
			<li><a href="selectProductList?fid=${f.pcId }">${f.pcName }</a></li>
			</c:otherwise>
			</c:choose>
			</c:forEach>
		</ul>
...
<div id="position" class="wrap">
	您现在的位置：<a href="indexSelect">创客空间-商城</a> &gt; <a href="selectProductList">商品分类</a> &gt; ${title }
</div>
<div id="main" class="wrap">
	<div class="lefter">
		<div class="box">
			...
			<dl>
				...
				<c:forEach var="f" items="${flist}">
					<dt><a href="selectProductList?fid=${f.pcId }">${f.pcName }</a></dt>
					<c:forEach var="c" items="${clist}">
						<c:if test="${f.pcId==c.pcParentId}">
							<c:if test="${p.pcChildId!=c.pcId}">
								<dd><a href="selectProductList?cid=${c.pcId }">${c.pcName }</a></dd>
							</c:if>
						</c:if>
					</c:forEach> 
				</c:forEach>
		...
		<div class="last-view">
			...
			<dl class="clearfix">
				<c:forEach var="lastp" items="${lastlylist}">
					<dt><a href="selectProductView?id=${lastp.prodId }"><img height="40" src="images/product/${lastp.prodFileName }" /></a></dt>
					<dd><a href="selectProductView?id=${lastp.prodId }">${lastp.prodName }</a></dd>
				</c:forEach>
			</dl>
		</div>
	</div>
	<div id="product" class="main">
		...
		<div class="infos">
		<div id="magnifier">
			<img src="images/product/${p.prodFileName }" id="img" />
			<div id="Browser"></div>
		</div>
		<div id="mag"><img id="magnifierImg" /></div>
			...
			<div class="buy">
				<p>商城价：<span class="price">￥${p.prodPrice }.00</span></p>
				<p>邮递方式：<font color="red">圆通快递</font></p>
				<p>热卖程度：☆☆☆☆☆</p>
				<p>库　存：<span id="stock">${p.prodStock }</span></p>
				...
				<div class="button"><input type="button" name="button" value="" onclick="shopAdd(${p.prodId})" /><a href="javascript:shopAdd2(${p.prodId})">放入购物车</a></div>
			</div>
			<div class="clear"></div>
		</div>
		<div class="introduce">
...
<div id="footer">
	Copyright &copy; 2018 创客空间 All Rights Reserved. 沪ICP备案000001号
</div>
</body>
</html>
